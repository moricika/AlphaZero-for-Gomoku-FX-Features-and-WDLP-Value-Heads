"""
Training script for 13×13 BASELINE AlphaZero Gomoku (no FX, no WDLP).

This is the standard AlphaZero baseline for comparison against the
FX+WDLP enhanced version. Same network architecture, same compute,
only difference is the absence of FX features and WDLP value head.

Designed to run SIMULTANEOUSLY with training_gomoku_13x13.py on a
single A800 80GB GPU (12 actors each, ~40GB per run).

Usage:
    python training_gomoku_13x13_baseline.py
"""
...
# Board and Network (same arch as FX+WDLP for fair comparison)
...
# NO FX, NO WDLP — this is the baseline
flags.DEFINE_bool('use_fx', False, 'FX features DISABLED for baseline.')
flags.DEFINE_bool('use_wdlp', False, 'WDLP value head DISABLED for baseline.')
flags.DEFINE_float('wdl_loss_weight', 0.01, 'Not used in baseline.')
flags.DEFINE_float('plies_loss_weight', 0.001, 'Not used in baseline.')
...
flags.DEFINE_string('ckpt_dir', './checkpoints/gomoku/13x13_baseline', 'Checkpoint directory.')
flags.DEFINE_string('logs_dir', './logs/gomoku/13x13_baseline', 'Logs directory.')
flags.DEFINE_string('eval_games_dir', '', 'Evaluation games directory.')
flags.DEFINE_string('save_sgf_dir', './games/selfplay_games/gomoku/13x13_baseline', 'SGF save directory.')
...
def main():
    ...
    logger.info('=' * 60)
    logger.info('  13×13 BASELINE AlphaZero Gomoku (no FX, no WDLP)')
    logger.info('=' * 60)
    ...
    logger.info(f'Input shape: {input_shape} (no FX enhancement)')
    ...
    def network_builder():
        return AlphaZeroNet(
            ...
            FLAGS.num_fc_units,
            gomoku=True,
            use_wdlp=False,  # BASELINE: standard MSE value head
        )

    network = network_builder()
    total_params = sum(p.numel() for p in network.parameters())
    logger.info(f'Network: {FLAGS.num_res_blocks} res blocks, {FLAGS.num_filters} filters')
    logger.info(f'Parameters: {total_params:,} (baseline, no WDLP head)')
    ...
    with mp.Manager() as manager:
        ...
        replay = UniformReplay(
            ...
            use_wdlp=False,  # BASELINE: no WDLP
        )
